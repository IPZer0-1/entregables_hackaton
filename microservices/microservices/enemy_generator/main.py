...
class SSLAdapter(HTTPAdapter):
    ...
    def init_poolmanager(self, *args, **kwargs):
        # Configura el uso de SSL en el pool de conexiones
        kwargs['ssl_context'] = self.context
        return super().init_poolmanager(*args, **kwargs)


def obtener_estadisticas(nombre: str) -> Dict:
    # Función para obtener estadísticas de un enemigo a partir de su nombre.
    try:
        # Convierte el nombre a formato de URL
        nombre_url = re.sub(r'[^a-zA-Z0-9\- ]', '', nombre).lower().replace(' ', '-')
        url = f"{BESTIARY_URL}{nombre_url}/"

        # Inicia una sesión HTTP personalizada
...
def obtener_lista_enemigos() -> List[Dict[str, str]]:
    # Función para obtener la lista de enemigos disponibles en el bestiario.
    try:
        session = requests.Session()
        session.mount('https://', SSLAdapter())
...
# Ruta para consultar la información de un enemigo en particular
async def consultar_enemigo(nombre: str = Body(..., embed=True)):
    enemigo = obtener_estadisticas(nombre)
    return {"enemigo": enemigo}


@app.get("/listar-enemigos/")
# Ruta para listar todos los enemigos disponibles
async def listar_enemigos():
